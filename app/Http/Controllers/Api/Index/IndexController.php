...
class IndexController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        /* 限时特卖 */
        $time_products= Product::limit(2)->orderBy('id','desc')->select(\DB::raw("id,title,CONCAT('".env('ATTACHMENT_URL')."',thumb) as thumb "))->where(['istime'=>1])->get()->toArray();
        ...
        /* 活动报名 */
        $active_articles = Article::limit(7)->orderBy('id','desc')->select(\DB::raw("id,article_linkurl,article_title,CONCAT('".env('ATTACHMENT_URL')."',resp_img) as resp_img "))->where(['article_category'=>4])->get()->toArray();
        ...
        /* 城里推荐 */
        $recommend_products= Product::limit(4)->orderBy('id','desc')->select(\DB::raw("id,title,CONCAT('".env('ATTACHMENT_URL')."',thumb) as thumb "))->where(['isindex'=>1,'isrecommand'=>1])->get()->toArray();
